...
async function run(args) {
  ...
  //   --next     same thing, except the checkpoint also decides WHICH group.
  //
  // In every case it's only trusted when it was built against these same inputs.
  let checkpoint = null;
  if (args.resume || args.group || args.next) {
  ...
  // `node build.js next` renders the first group the checkpoint hasn't
  ...
  if (args.next && !groupId) {
    ...
    groupId = pending[0];
    log.line('render', 'next up: ' + groupId + ' (' + pending.length + ' pending)');
  }

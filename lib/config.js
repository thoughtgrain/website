...
// Two human-readable mirrors of what the build is doing. They exist because a
// terminal is not guaranteed: in Code App on iOS I get no stdout at all, so
// every progress line is also appended to BUILD_LOG_PATH, and STATUS_PATH holds
// a short "here's what's done and what's left" snapshot rewritten after every
// group. Both are openable in an editor pane, which works even when the
// terminal doesn't.
const BUILD_LOG_PATH = path.join(CACHE, 'build.log');
const STATUS_PATH = path.join(CACHE, 'status.txt');

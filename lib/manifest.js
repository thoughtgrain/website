...
// The non-content sections, in the order they're reported. Kept in one place so
// `staleness` and `changedKinds` can't drift apart about what a manifest holds.
const FLAT_SECTIONS = ['layouts', 'components', 'css', 'js', 'data', 'assets'];

/**
 * Walk every section of a manifest, calling `visit(sectionName, entries)`.
 *
 * Content collections are keyed under `content`; everything else sits at the top
 * level. Both are reported by their plain section name, which is the vocabulary
 * the page modules use in their `inputs` declarations.
 */
function eachSection(manifest, visit) {
  Object.keys(manifest.content || {}).forEach(function (key) {
    visit(key, manifest.content[key]);
  });
  FLAT_SECTIONS.forEach(function (key) {
    visit(key, manifest[key] || []);
  });
}

/** One entry's identity for comparison purposes: where, how big, when. */
...
function staleness(manifest) {
  ...
  eachSection(manifest, function (section, entries) {
...
/**
 * Which sections differ between two manifests.
 *
 * This is what lets `next` rebuild the thing you actually edited rather than
 * restarting from the top of the pipeline. Comparing two manifests — rather than
 * a manifest against disk — means an ADDED or DELETED file counts too, which a
 * per-entry staleness check can't see.
 *
 * @param {object} before
 * @param {object} after
 * @returns {string[]} Section names, e.g. ['notes'] or ['layouts', 'css'].
 */
function changedKinds(before, after) {
  if (!before || !after) return [];

  const snapshot = function (manifest) {
    const sections = {};
    eachSection(manifest, function (section, entries) {
      sections[section] = entries.map(entryKey).sort().join('|');
    });
    return sections;
  };
  ...
  // Union of both key sets, so a section that appeared or vanished still counts.
  const names = Object.keys(a).concat(Object.keys(b)).filter(function (k, i, all) {
    return all.indexOf(k) === i;
  });

  return names.filter(function (name) { return a[name] !== b[name]; }).sort();
}

module.exports = {
  VERSION, scan, save, load, fingerprint, staleness, changedKinds, eachSection,
};
